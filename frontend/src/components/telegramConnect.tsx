...
export default function TelegramConnect() {
  ...
  // TanStack Query Mutation for fetching / generating the link code
  const {
    mutate: generateCode,
  ...
  // Trigger code generation on mount
  useEffect(() => {
    generateCode();
    return () => stopPolling();
  }, [generateCode]);
        ...
        <div className="bg-[#181B1F] border border-[#22252B] rounded-xl shadow-[0_16px_40px_rgba(0,0,0,0.6)] p-6 sm:p-8 font-mono">
          ...
          {isCodeError && (
            <div className="mb-6 p-3.5 bg-red-500/10 border border-red-500/30 rounded-lg text-xs text-red-300">
              {codeError instanceof Error ? codeError.message : 'Could not generate a code. Please try again.'}
              <button
          ...
          {!isGeneratingCode && !isCodeError && code && (
            <div className="space-y-4 mb-6 text-xs">
              ...
              <div className="p-3.5 bg-[#0B0C10] border border-[#22252B] rounded-lg text-slate-300">
                ...
                We'll detect the link automatically — redirecting upon connection.
              </div>
            </div>
          )}
